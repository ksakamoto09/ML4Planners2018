...
Sometimes two variables interact and are important features to capture. In R, you either use
`*` or `:` to create unique combiniations of predictor variables on the dependent variable.
The difference between the two is that `*` will keep the original terms while `:` will only keep
the interaction terms and no the original terms. 
...
Here is a table of the different types of generalized linear models that could be fit using
the `glm` function. 
![](https://www.statisticshowto.datasciencecentral.com/wp-content/uploads/2014/01/link-function.jpg)
