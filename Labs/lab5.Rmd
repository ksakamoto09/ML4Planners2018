...
backwards or forwards usually bad
...
```{r}
Lots <- readr::read_csv(file.path(dataDir, "NYC_Lots.csv"))
MN <- Lots %>% filter(Borough == "MN" , LandUse %in% c(10, 06)) %>% 
    mutate(Parking = if_else(LandUse == 10, "Parking", "Manufacturing") %>% as.factor) %>% 
    mutate(LandUse = as.character(LandUse)) %>% 
    select( LandUse, LotArea, AssessLand, NumFloors, Parking)
```

```{r}
parking1 <- glm(Parking ~  scale(LotArea) + scale(AssessLand) + NumFloors , data = MN,
