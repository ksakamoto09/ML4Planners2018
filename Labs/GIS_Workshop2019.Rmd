...
Here's another graphic showing that if the cylinder fits just around the equator, there is a point of tangency. On the otherhand,
...
distoration, meaning that there is more object than space on the developable surface, while outside of the
parellels there is positive distortion because there is less objectfor the surface and gets blown up. 
...
- Cliamte maps can show atmostpheric condtions
...
maps are for thematic mapping and should be thought of more as data visulation.
...
- Legend : Gives an undestanding of the range, and variation in data (make the units easy to read)
- North Arrow : Offers orientation for caridinal directions
...
Not all maps are created eqaul and honing a critical eye towards good data visualzation
and graphic design will take you far. Also once you create your aestheic whether it's 
more modern, or tradiational looking, it'll be easier to stick with it. 
...
We can also use a variable(either continous or categorical) to change the color too.
...
such as Manhattan. So even though Manahttan is not perfectly vertically aligned with North
a lot of dpecition show it as such.
...
hand-coded these vectors from often statellite imagery or historic maps. The level
...
The issue wwith our previous function is that topology between geometries
...
Centroids are also known as the geometric center which is the artithmetic mean position of all the points that consist of a geometry. 
...
the interserction of a half-mile from the school and a half-mile from a public library?
...
The foundations of point pattern analysis is the summary statistcs that we
...
```{r}
aspace::calc_sdd(id = 1, filename = "Data/sdd.txt", points = data.frame(cipPPP$x, cipPPP$y))
```
...
- compare two or more distrubutions
...
We looked global density, but what about point patterns that cane be measured across differenct location within the study region?
...
We can first create a 3x3 quadrat using the `quadratcount` funciton which will divide
up our schools and count up the number of schools.
...
```{r}
st_join(cipSF, nybb) %>% group_by(BoroName) %>% 
    summarise(schools = n(),
              area = first(Shape_Area),
              density = schools/area) %>% 
    arrange(desc(density))
```

## Kerenel Density

The kernel density is a method for exploring desnity from sampled features. A circula search area
is applied that determines the distance to search for sample locations to spread values around the
locations. 


We will now calculate the density and plot it out with the default values. we are
using the `density` function from `spatstat`. The default kernel is the "gaussian smoothin kernel"
