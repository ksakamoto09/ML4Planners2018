...
- there is relatively low natural/inexpiable random variation.
...
Autocorrelation is the strength of the relationship with previous observations of the data.
...
similarly to the naive method, the forecast is equal to the last observed number from the same season a year ago. *m* is the seasonal period and *k* is the integer part of (*h* - 1)/*m*
...
it is not staying around a mean. Although our histogram shows normality, which will give us
...
When we get the back-transformed forecast from the Box-Cox transformation, it will return the median of the forecast distribution rather than the mean. The difference between the mean and the median is called the **bias** hence **bias-adjusted** point forecasts. 
...
If we consider the average method...
...
from our time series will be weighted less.
...
The next component to add is seasonality. We are adding one more smoothing equation for $s_t$. We will use $m$ to denote
the frequency of the seasonality, so quarterly seasonality will be $m = 4$ or monthly would be $m = 12$.
...
A **Stationary** time series is one whose Properties do not depend on the times t which the series is observed.
This means that time series with trends and seasonality are not considered to be stationary. Remember cyclical patterns?
Those are considered stationary series. A stationary series will have no predictable pattern in the the long run.
...
**Differencing** is when you compute the differences between consecutive observations. This is another form of transformation that can turn non-stationary data into stationary. This technique can help stabilize the mean of a time series by removing the changes in the level(removing the Trend and seasonality). 

Occasionally the differenced data is not stationary and may need to be differenced a second time. This is called a **Second-Order Differencing** when you difference a second time. 
...
of predictors, the autoregressive model uses a linear combination of past values of the variable. Remember for time 
...
Unline the autoregressive model, a moving average model uses past forecast errors in a regression like model.
...
Non-seasonal ARIMA models are when you combine differencing with autroregression and a moving average model.
ARIMA stands for AutoRegressive Integrated Moving Average, where integration is the reverses of differenceing. 
...
Usually determining $p$:
- The ACF is exponentially decaying or sinusoidal;
- There is a significant spike at lap $p$ in the PACF, but none beyond lag $p$

Usually determining $q$:
- The PACF is exponentially decaying or sinusoidal;
- There is a significant spike at lap $q$ in the ACF, but none beyond lag $q$
