...
Geography was essential to the survival of humans from the early hunter-gathers 
locating their food supply, water, and housing. Humans have been exploring and 
cataloguing the world around them, through collective memory,
etched on clay tablets, or stored on data bases. We humans no longer need to take
inventory of our natural surroundings quite like our anciet ancestors, but the need 
to navigate spatial information has only increased. A myriad of
apps require a geographic location, such as ride sharing apps like Uber
and Lyft, tracking packages from Amazon and other e-retailers, and even posting/check-ins
on social media apps like Instagram and Facebook.

![Imago Mundi: Babylonia 500 BCE](https://www.geolounge.com/wp-content/uploads/2014/11/Baylonianmaps.jpg)

Cartogprahy completely changed in 1854 with John Snow's Cholera map. For the first time 
data and summary statistics were combined on to a map. You can see that there is a barchart
on the streets showing the number of deaths per building.

![](https://upload.wikimedia.org/wikipedia/commons/thumb/2/27/Snow-cholera-map-1.jpg/300px-Snow-cholera-map-1.jpg)
...
was able to become more widespread hearlding in many more users. Unlike most tabular 
data, spatial data is still unique which is why GIS is important in unlocking
insights from geography. 

## Geographic Data Models

The two most fundamental data models used in geographic data is **vector** and **raster**.
If you are familiar with the Adobe Creative Suite programs, **vector** will be analgous
...
![](../Images/gisDataModel.png)

### Vector


![](../Images/SpatialDataRepresentation.png)
### Raster
...
In normal autocorrelation, things like your weight change gradually over time so weight is highly correalted with the days that are consecutive. In spatial autocorrelation we try to see how a location, whether it is a polygon, point, or raster grid are similar to eachother. 
...
The values of $I$ usually range from -1 to 1. This shouldn't come as a suprise if you think of values that get outputed when we talk about correlations between two variables, there is usually a negative correlation (-1), no correlation (0), or positive correlation (1). In Moran's I -1 means evenly dispersed, 0 is random, and 1 is perfectly clustered. 
